...
from torch.nn import (Conv2d, BatchNorm2d, MaxPool2d,
                      AvgPool2d, ReLU, BatchNorm1d, Linear, CrossEntropyLoss)
...
class DownConv(Module):
    def __init__(self, in_feat, out_feat, config, drop_rate=0.4, bn_momentum=0.1, firstlayer=False):
        ...
        if config['quantization'].lower() != "normal" and not firstlayer:
            self.conv1 = Conv2dQuant(in_feat, out_feat, kernel_size=(
                3, 3), padding=1, H=1, W_LR_scale="Glorot", config=config, stride=1)
            self.conv2 = Conv2dQuant(out_feat, out_feat, kernel_size=(
                3, 3), padding=1, H=1, W_LR_scale="Glorot", config=config, stride=1)
        else:
            self.conv1 = nn.Conv2d(
                in_feat, out_feat, kernel_size=3, padding=1, stride=1)
            self.conv2 = nn.Conv2d(
                out_feat, out_feat, kernel_size=3, padding=1, stride=1)
...
class UpConv(Module):
    def __init__(self, in_feat, out_feat, config, drop_rate=0.4, bn_momentum=0.1, beta=2.0):
        ...
        self.downconv = DownConv(in_feat=in_feat, out_feat=out_feat,
                                 drop_rate=drop_rate, bn_momentum=bn_momentum, config=config)
...
class Unet(Module):
    ...
    def __init__(self, config, drop_rate=0.4, bn_momentum=0.1):
        ...
        self.beta = 3.0
        # Downsampling path
        self.conv1 = DownConv(1, 64, drop_rate=drop_rate,
                              bn_momentum=bn_momentum, config=config, firstlayer=True)
        #self.conv1 = DownConv(1, 64, drop_rate=drop_rate, bn_momentum=bn_momentum, config=config, firstlayer=False)
        self.mp1 = nn.MaxPool2d(2)

        self.conv2 = DownConv(64, 128, drop_rate=drop_rate,
                              bn_momentum=bn_momentum, config=config)
        self.mp2 = nn.MaxPool2d(2)

        self.conv3 = DownConv(128, 256, drop_rate=drop_rate,
                              bn_momentum=bn_momentum, config=config)
        self.mp3 = nn.MaxPool2d(2)

        # Bottom
        self.conv4 = DownConv(256, 256, drop_rate=drop_rate,
                              bn_momentum=bn_momentum, config=config)

        # Upsampling path
        self.up1 = UpConv(512, 256, drop_rate=drop_rate,
                          bn_momentum=bn_momentum, config=config)
        self.up2 = UpConv(384, 128, drop_rate=drop_rate,
                          bn_momentum=bn_momentum, config=config)
        self.up3 = UpConv(192, 64, drop_rate=drop_rate,
                          bn_momentum=bn_momentum, config=config)
