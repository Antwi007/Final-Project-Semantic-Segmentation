...
class SCGMChallenge2DTrain(MRI2DSegmentationDataset):
    ...
    def __init__(self, root_dir, slice_axis=2, site_ids=None,
        ...
        self.site_ids = site_ids or range(
            1, SCGMChallenge2DTrain.NUM_SITES + 1)
        self.subj_ids = subj_ids or range(
            1, SCGMChallenge2DTrain.NUM_SUBJECTS + 1)
        self.rater_ids = rater_ids or range(
            1, SCGMChallenge2DTrain.NUM_RATERS + 1)
            ...
            for subj_id in self.subj_ids:
                if len(self.rater_ids) > 0:
                    for rater_id in self.rater_ids:
                        input_filename = self._build_train_input_filename(
                            site_id, subj_id)
                        gt_filename = self._build_train_input_filename(
                            site_id, subj_id, rater_id)

                        input_filename = os.path.join(
                            self.root_dir, input_filename)
...
class SCGMChallenge2DTest(MRI2DSegmentationDataset):
    ...
    def __init__(self, root_dir, slice_axis=2, site_ids=None,
        ...
        self.subj_ids = subj_ids or range(
            11, 10 + SCGMChallenge2DTest.NUM_SUBJECTS + 1)
        ...
        for site_id in self.site_ids:
            for subj_id in self.subj_ids:
                ...
                if not os.path.exists(input_filename):
                    raise RuntimeError(
                        "Path '{}' doesn't exist !".format(input_filename))
                self.filename_pairs.append((input_filename, gt_filename))
