...
def to_fixed_point(v, i, f):
    ...
    max_float = Variable(torch.zeros(v.size()).type(v.data.type()).cuda() + torch.pow(2*torch.ones(pows.size()).cuda(),
                         pows.float().cuda()).sum()) if f != 0 else Variable(torch.zeros(v.size()).type(v.data.type()).cuda())
    max_int = Variable(torch.zeros(v.size()).type(v.data.type()).cuda(
    ) + 2**(i-1)) if i != 0 else Variable(torch.zeros(v.size()).type(v.data.type()).cuda())
...
#     # v = v.cuda()
#     pows = torch.arange(-f, 0, 1)
#     max_float = Variable(torch.zeros(v.size()).type(v.data.type()) + torch.pow(2*torch.ones(pows.size()),
#                          pows.float()).sum()) if f != 0 else Variable(torch.zeros(v.size()).type(v.data.type()))
#     max_int = Variable(torch.zeros(v.size()).type(v.data.type(
#     )) + 2**(i-1)) if i != 0 else Variable(torch.zeros(v.size()).type(v.data.type()))
...
class Ternary(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ...
        w_in = x
        a, b, c, d = w_in.size()
        delta = 0.7*torch.mean(torch.mean(torch.mean(torch.abs(w_in),
                               dim=3), dim=2), dim=1).view(-1, 1, 1, 1)
